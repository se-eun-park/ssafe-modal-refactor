  ...
  return (
    <div className="relative w-[67.2rem] h-[52.5rem] rounded-md border-solid border-2 border-black-500 flex flex-col bg-white">
      ...
      <div className='w-[60.8rem] h-[33.6rem] flex flex-col rounded-md mx-auto mt-[2.4rem] bg-[#FAFAFA] border-solid border-2 border-black-500'>
        ...
        <input id="surveyTitle" className='w-[57.6rem] h-[3.2rem] rounded-md mt-[0.8rem] mx-auto bg-[#F5F5F5] border-solid border-2 border-black-500' onChange={(e) => setTitle(e.target.value)} placeholder='제목을 입력하세요' ></input>
        <label htmlFor="surveyContent" className='ms-[1.6rem] mt-[3.2rem] text-[1.4rem]'>설문지 설명을 입력하세요</label>
        <textarea id='surveyContent' className='w-[57.6rem] h-[9.6rem] rounded-md mt-[0.8rem] mx-auto bg-[#F5F5F5] border-solid border-2 border-black-500' onChange={(e) => setContent(e.target.value)} placeholder='내용을 입력하세요' ></textarea>
        <div className='w-[57.6rem] h-[3.2rem] flex flex-row mx-auto mt-[3.2rem] justify-start' >
          {tagList.map(
            tag => (<SurveyTag tagText={tag}></SurveyTag>)
          )}
          <input className={`w-[10rem] h-[3.2rem] bg-[#D4D4D4] grid place-items-center text-white rounded-md text-[1.2rem] font-bold text-center ${tagList.length > 4 ? 'hidden' : ''}`} onChange={(e) => setTag(e.target.value)} onKeyDown={(e) => {
            if (e.key === 'Enter')
              makeTag()
          }} maxLength={10} placeholder='#태그입력' value={tag}/>
        </div>
      </div>
