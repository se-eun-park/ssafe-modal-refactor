...
const Modal = ({ modalType, setModalOpen }: { modalType: string, setModalOpen: React.Dispatch<SetStateAction<boolean>> }) => {
  return (
...
const LoginModal = ({ setModalOpen }: { setModalOpen: React.Dispatch<SetStateAction<boolean>> }) => {
  ...
  return (
    <div className="relative w-[48rem] h-[23rem] rounded-md border-solid border-2 border-black-500 flex flex-col">
      ...
      <button className='absolute bottom-2 right-3 w-[4rem] h-[3rem] mt-[3.2rem] rounded-md text-[1.4rem] font-bold border-solid border-2 border-black-600/100' onClick={handleNextBtn}>확인</button>
    </div>
